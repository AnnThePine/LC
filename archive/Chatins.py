...
def merge_close_peaks(energies):
    """
    Merges peaks that are within `tol` of each other by averaging their values.
    
    Args:
        energies (list or np.array): A list of energy values.
    
    Returns:
        list: A list of merged energy values.
    """
    if len(energies) == 0:
        return []
    ...
    for e in energies[1:]:
        if abs(e - merged[-1]) <= tol:
            # If the current energy is within tolerance of the last merged peak,
            # merge them by averaging.
            merged[-1] = (merged[-1] + e) / 2
        else:
            # Otherwise, start a new merged peak.
            merged.append(e)
    return merged
...
for alpha in range(alpha_min, alpha_max, ik_pec_lenkis):
    ...
    # Store the results for the current lauks value
    area_results.append(( total_area))
    area_24_only_results.append((total_area_24_only))
...
plt.bar(lauks_values,area_24_only_results)
plt.show()